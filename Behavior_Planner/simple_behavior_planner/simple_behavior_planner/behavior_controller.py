...
class BehaviorController:
    ...
    def _init_parameter_manager(self, config_path: str):
        ...
        try:
            ...
            self.node.get_logger().debug(f"Behavior controller initialized with config: {full_config_path}")
    ...
    def _log_available_behaviors(self):
        ...
        self.node.get_logger().debug(f"Available behaviors: {len(behaviors)}")
    ...
    def _log_parameter_change(self, node_type: int, params: dict):
        ...
        self.node.get_logger().debug(f"🔄 [BEHAVIOR UPDATE] {self.current_node_type}->{node_type}: {behavior_desc}")
        for param, value in key_params.items():
            if value != 'N/A':
                self.node.get_logger().debug(f"   {param}: {value}")
