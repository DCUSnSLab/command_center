# === sa_mppi_optimizer.py (DIAGNOSTIC PATCH) ===
...
class SMPPIOptimizer:
    def __init__(self, params: dict):
        ...
        # === SA-MPPI runtime adaptation state ===
        # base_* hold the nominal proposal p and limits; runtime_* hold the
        # situation-adapted proposal q_phi. Equal by default, so with no
        # adaptation SA-MPPI reduces EXACTLY to plain SMPPI (log_is == 0 below).
        self.base_noise_std = self.noise_std.clone()
        self.base_temperature = float(self.temperature)
        self.base_v_max = float(self.v_max)
        self.base_v_min = float(self.v_min)
        self.runtime_noise_std = self.base_noise_std.clone()
    ...
    def set_runtime_adaptation(self, profile=None):
        ...
        profile keys (all optional, default = identity):
          noise_scale_v, noise_scale_delta, lambda_scale, speed_scale,
          reverse_scale, v_bias.
        An empty/None profile restores the nominal proposal p => plain SMPPI.
        """
        if not profile:
            profile = {}
        ...
        self.runtime_noise_std[0] = self.base_noise_std[0] * noise_scale_v
        self.runtime_noise_std[1] = self.base_noise_std[1] * noise_scale_delta
    ...
    def get_runtime_velocity_limits(self):
        """Velocity clamp bounds after runtime speed scaling (situation speed cap
        incl. braking-distance CBF computed by the SA layer). Returns (v_min, v_max)."""
    ...
    def get_nominal_plan_A(self) -> torch.Tensor:
        """Integrate the nominal derivative-control sequence U into the absolute
        [v, delta] plan [T,2], for the situation layer's curvature (internal) score."""
        a0 = self.last_cmd_applied
        return self._integrate_U_to_A(a0, self.control_sequence.unsqueeze(0))[0]
    ...
    def optimize(self) -> torch.Tensor:
        ...
        for it in range(self.iteration_count):
            ...
            # importance weights (SA-MPPI: situation-aware IS correction)
            # Samples are drawn from the adapted proposal q_phi ~ N(U_nom, runtime_noise_std).
            # The nominal proposal is p ~ N(U_nom, base_noise_std). Adding the
            # log-likelihood ratio (log p - log q_phi) over the noise eps de-biases
            # the exponential weighting (generalized IS, Biased-MPPI 2024). When
            # runtime == base (no adaptation) log_is == 0 -> identical to plain SMPPI.
            sig_q = self.runtime_noise_std.view(1, 1, 2)
            sig_p = self.base_noise_std.view(1, 1, 2)
            log_is = (0.5 * (eps / sig_q) ** 2).sum(dim=(1, 2)) \
                   - (0.5 * (eps / sig_p) ** 2).sum(dim=(1, 2))   # [K] = log p - log q_phi
            exp_arg = -(total_costs) / max(1e-9, self.runtime_temperature) + log_is
            exp_arg = exp_arg - exp_arg.max()   # numerical stability (cancels in softmax)
            ...
            with torch.no_grad():
                ...
                A_preview[:, 0] = torch.clamp(A_preview[:, 0], _vmin_rt, _vmax_rt)
    ...
    def _sample_U_and_build_A(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # SA-MPPI: sample from the runtime (situation-adapted) proposal std.
        noise = torch.randn(self.K, self.T, 2, device=self.device, dtype=self.dtype) * self.runtime_noise_std
        ...
        # SA-MPPI: apply situation-adapted velocity limits (speed cap / CBF).
        v_min_rt, v_max_rt = self.get_runtime_velocity_limits()
        a0[0] = torch.clamp(a0[0], v_min_rt, v_max_rt)
        a0[1] = torch.clamp(a0[1], self.w_min, self.w_max)     # δ 한계 보장
        A_samples = self._integrate_U_to_A(a0, U_samples)

        # 속도 한계 적용
        A_samples[..., 0] = torch.clamp(A_samples[..., 0], v_min_rt, v_max_rt)
    ...
    def get_control_command(self) -> Twist:
        ...
        _vmin_rt, _vmax_rt = self.get_runtime_velocity_limits()  # SA-MPPI runtime speed cap
        A[:, 0] = torch.clamp(A[:, 0], _vmin_rt, _vmax_rt)
