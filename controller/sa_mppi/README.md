SMPPI Controller 코드 분석
  ...
  sa_mppi/
  ├── scripts/
  │   ├── sa_mppi_controller_node.py          # 메인 ROS2 노드
  ...
  ├── sa_mppi_controller/
  │   ├── optimizer/
  │   │   ├── sa_mppi_optimizer.py           # 핵심 MPPI 최적화 로직
  ...
  **Model Predictive Path Integral (MPPI)**는 확률적 최적 제어 알고리즘입니다:
  ...
  ---
  🔥 메인 컨트롤러 노드 (sa_mppi_controller_node.py)

  sa_mppi_controller_node.py:43-669
  ...
  - /sa_mppi_visualization - RViz 마커

  제어 루프 (control_callback)

  sa_mppi_controller_node.py:336-412
  ...
  ---
  ⚙️ SMPPI Optimizer (sa_mppi_optimizer.py)

  sa_mppi_optimizer.py:25-504
